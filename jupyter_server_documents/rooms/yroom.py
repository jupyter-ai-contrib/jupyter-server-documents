...
class _ExecutionItem:
    """A cell execution request queued for the YRoom execution worker."""
    cell_id: str
    ycell: Any
    file_id: str
    clear_outputs: bool
...
class YRoom(LoggingConfigurable):
    ...
    async def connect_kernel(self, kernel_manager) -> None:
        """Attach this YRoom to a running kernel.

        If already connected, disconnects cleanly before reconnecting.
        Creates a fresh AsyncKernelClient (independent session to avoid ZMQ
        DEALER identity collisions), waits for heartbeat, starts the execution
    ...
    async def execute_cell(
        ...
        if self._execution_queue is None:
            raise RuntimeError("YRoom execution worker is not running")
