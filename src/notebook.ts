        ...
        if ('insert' in outputsChange) {
          ...
          for (const output of outputsChange.insert!) {
            ...
            if('toJSON' in output){
              ...
              console.debug("output from doc: ", parsed)
              const metadata = parsed.metadata
              if(metadata.output_url){
                if(metadata.output_type === "stream"){
                  // fetch the real output
                  requestAPI(metadata.output_url).then((data) => {
                    console.debug("output from outputs service: ", data)
                    // @ts-ignore
                    this._outputs.clear()
                    // @ts-ignore
                    this._outputs.add({
