...
from jupyter_rtc_core.kernels.message_cache import KernelMessageCache, MissingKeyException  # Replace your_module

@pytest.fixture
def cache():
    return KernelMessageCache()
...
def test_setitem_and_getitem(cache):
    message1 = create_message("msg1", "shell")
    cache["msg1"] = message1
    assert cache["msg1"] == message1


def test_setitem_key_mismatch(cache):
    message1 = create_message("msg1", "shell")
    with pytest.raises(Exception, match="Key must match `msg_id` in value"):
        cache["wrong_key"] = message1


def test_setitem_missing_msg_id(cache):
    message1 = {"channel": "shell"}  # Missing msg_id
    with pytest.raises(MissingKeyException, match="`msg_id` missing in message data"):
        cache["key"] = message1


def test_setitem_missing_channel(cache):
    message1 = {"msg_id": "msg1"}  # Missing channel
    with pytest.raises(Exception, match="`channel` missing in message data"):
        cache["msg1"] = message1


def test_setitem_with_cell_id(cache):
...
def test_setitem_without_cell_id(cache):
    message1 = create_message("msg1", "shell")
    cache["msg1"] = message1
    assert "msg1" in cache._by_msg_id
...
def test_delitem_with_cell_id(cache):
    message1 = create_message("msg1", "shell", "cell1")
    cache["msg1"] = message1
    del cache["msg1"]
    assert "msg1" not in cache
...
def test_remove_by_msg_id(cache):
    message1 = create_message("msg1", "shell")
    cache["msg1"] = message1
...
def test_remove_by_cell_id(cache):
    message1 = create_message("msg1", "shell", "cell1")
    cache["msg1"] = message1
...
def test_pop_nonexistent(cache):
...
def test_maxsize_eviction_with_cell_id(cache):
    cache = KernelMessageCache(maxsize=2)
    message1 = create_message("msg1", "shell", "cell1")
